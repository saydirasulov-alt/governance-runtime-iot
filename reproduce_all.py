"""
Reproduce the governance-overhead backends, the appendix ablations, the tamper-evident
audit chain, and the policy-loader / OPA conformance -- i.e. the tables that do not require
the physical-plant integration -- on this machine, in one command.
...
The Tier-3 latency on a real MQTT stack is reproduced by   python run_governance_demo.py .
"""
...
def main():
    ...
    log += ["", f"total wall clock : {total/60:.1f} min",
            ...
            "",
            "The backend, ablation, gate-cost, corrected-G2, AI-failure, audit-chain and OPA-conformance",
           "values above are produced on this machine. The closed-loop, sweep and robustness",
           "numbers come from  python sil_testbed/run_all.py ; the real-MQTT prototype from",
           "python run_governance_demo.py ; the OPA HTTP latency from  python experiments/opa_http_latency.py .",
            "=" * 78]
