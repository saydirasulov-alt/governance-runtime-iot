...
def main():
    ...
    try:
        ...
        sys.exit(f"OPA server not reachable at {base} ({e}).\n"
                 f"Start one first, e.g.:  docker run --rm -p 8181:8181 openpolicyagent/opa:latest run --server")
    ...
    print("=" * 70)
    print("OPA HTTP decision-point latency")
    print("=" * 70)
    print(f"host            : {platform.node()}")
    print(f"os              : {platform.system()} {platform.release()} ({platform.machine()})")
    print(f"python          : {sys.version.split()[0]}")
    print(f"opa endpoint    : {base}/v1/data/{ALLOW_PATH}")
    print(f"intents (timed) : {len(rows)}  (admitted {admitted}, rejected {len(rows)-admitted}; warmup {a.warmup})")
    print(f"decision agree  : {len(rows)-mism}/{len(rows)}  (OPA vs runtime gate logic)")
    print(f"latency median  : {statistics.median(lat):.3f} ms")
    print(f"latency P90     : {pct(90):.3f} ms")
    print(f"latency mean    : {statistics.mean(lat):.3f} ms   (min {lat[0]:.3f}, max {lat[-1]:.3f})")
    print(f"csv             : {a.out}")
    if mism:
